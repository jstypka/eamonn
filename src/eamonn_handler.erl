...
-spec handle(Req :: #req{}, callback_args()) ->
                    ignore |
                    {response_code(), [tuple()], binary()} |
                    {ok, [tuple()], binary()}.
handle(Req, _Args) ->
...
-spec handle(http_method(), [binary()], #req{}) ->
                    ignore |
                    {response_code(), [tuple()], binary()} |
                    {ok, [tuple()], binary()}.
handle('GET', Filepath, _Req) ->
    Concatenated = lists:foldr(fun(Elem, Path) ->
                                       << Elem/binary,
                                          <<"/">>/binary,
                                          Path/binary >>
                               end, <<>>, Filepath),
...
%% tree.
-spec handle_event(Event :: elli_event(),
                   Args :: [term()],
                   Config :: [tuple()]) -> ok.
...
handle_event(request_error, [_Request, _Exception, _Stacktrace], _) ->
    ok;

handle_event(request_exit, [_Request, _Exception, _Stacktrace], _) ->
    ok;
...
handle_event(invalid_return, [_Request, _ReturnValue], _) ->
    ok;
...
handle_event(client_closed, [_Where], _) ->
    ok;
...
handle_event(client_timeout, [_Where], _) ->
    ok;
